...
Han Eap Simulator Web UI 是半导体设备自动化模拟系统的前端界面。它面向 SECS/GEM、HSMS、EAP 联调场景，提供设备连接管理、SML 报文维护、手动收发、自动回复、可视化 Flow 编排，以及 Checklist 驱动的 SML/Flow 生成能力。

本仓库只包含前端源码。后端服务、数据库、迁移脚本、种子数据、Checklist 模板和完整发布包由主项目单独维护。
...
新数据库首次打开会进入初始化页面，请在页面中为内置管理员账号设置密码：
...
默认开发服务器由 Vite 提供，通常运行在 `http://localhost:3000`。后端 API 默认地址由前端运行时配置和 `src/utils/api.ts` 决定，开发时通常指向 `http://localhost:8080/api`。
...
本仓库中的前端源码和前端文档使用 MIT License。后端源码、预构建后端二进制、完整应用发行包、数据库迁移、种子数据和 Checklist 模板不包含在本授权范围内，需遵循各自单独的分发条款。
